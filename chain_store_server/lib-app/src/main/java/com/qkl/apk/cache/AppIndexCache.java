 ...
 **/
@Component
public class AppIndexCache extends AbstractCache<String> {


    public void saveTodayApp(String date,  List<ApkDto> apkDtoList){
        String str=get(date);
        Assert.isTrue(apkDtoList.size()<=CodeConstant.INNDEX_APP_LIST_SIZE,"首页APP不能超过3个APP");
        ...
        if(StringUtil.isNotNil(str)){
            ...
            Assert.isTrue(cacheApkList.size()<=CodeConstant.INNDEX_APP_LIST_SIZE,"首页APP不能超过3个APP");
