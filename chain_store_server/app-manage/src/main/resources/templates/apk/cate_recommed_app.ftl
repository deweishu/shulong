        ...
        <div class="col-sm-12">
            <div class="ibox ">
                ...
                <div class="ibox-content">
                    <p>
                        <button class="btn btn-info " type="button" onclick="addRecommend();"><i class="fa fa-plus"></i>&nbsp;添加新推荐</button>&nbsp;&nbsp;
                    </p>
                    <hr>
...
<input type="hidden" name="apkIds" id="apkIds" />
</body>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#table_list").bootstrapTable({
            ...
            //获取数据的Servlet地址
            url: "${ctx!}/cate/recommed/data/${cateId!}",
            //表格显示条纹
            striped: true,
            //启动分页
            pagination: false,
    ...
    function removeToday(id) {
        ...
        layer.confirm(t, {icon: 3, title:'提示'}, function(index){
            ...
            $.ajax({
                type: "GET",
                dataType: "json",
                url: "${ctx!}/cate/recommed/delete/"+cateId+"/"+id,
    ...
    function addRecommend(){
        ...
        layer.open({
            ...
            closeBtn: 1,
            shadeClose: false,
            content: '${ctx!}/apk/select/page/mult',
            end: function(index){
                var ids=$("#apkIds").val();
                if(ids.length>0){
                    layer.confirm("确定添加到该分类推荐吗？", {icon: 3, title:'提示'}, function(index){
                        var load=layer.load(1);
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            contentType: "application/json;charset=utf-8",
                            data : JSON.stringify({apkIds:ids,cateId:cateId}),
                            url: "${ctx!}/cate/recommed/add",
                            success: function(msg){
